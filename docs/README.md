...
| Backend | **None.** No server, no database, no API. | Express + MongoDB + Google Places, etc. |
| Matches/News | **Static**, taken manually from FIFA's site. | Synced automatically. |
| Hosting | Next.js **static export** on Vercel. | Same web app + an API service + DB. |
...
| 12 | [Infrastructure](./12-infrastructure.md) | 1 | Static export → Vercel; contact form service |
...
| 06 | [Modules](./06-modules.md) | **2+** | *Future* backend modules |
| 07 | [Database](./07-database.md) | **2+** | *Future* MongoDB design |
| 08 | [API Spec](./08-api-spec.md) | **2+** | *Future* REST API |
| 09 | [Integrations](./09-integrations.md) | 1+2 | Phase 1 links; *future* Places/Maps APIs |
| 10 | [Admin / CMS](./10-admin-cms.md) | **2+** | *Future* content management |

## Tech stack

- **Phase 1:** Next.js 15 (App Router, **static export**) · TypeScript · TailwindCSS · shadcn/ui. Content in local `data/` files. Contact form via a no-backend form service (Formspree / Web3Forms). Deploy on **Vercel**.
- **Phase 2+:** add Express + MongoDB Atlas + Mongoose + a custom CMS, swapping hardcoded data for live data — **no rewrite of the pages**, just the data source.
...
4. **Content-first, low-chrome design** — clean, legible, *not* AI-generated-looking; no decorative shadows/gradients. ([principles](./04-ux-design.md#design-principles))
