...
export default function AdminDashboardPage() {
  ...
  async function onAdd(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        setMsg({
          kind: "err",
          text: "Could not add item. Check fields (URLs must be valid).",
        });
      }
    } catch {
  ...
  async function onApproveContact(id: string, approved: boolean) {
    ...
    if (res.ok) {
      ...
      setMsg({
        kind: "ok",
        text: approved ? "Story approved and published." : "Story unapproved.",
      });
    } else if (res.status === 401) {
      clearToken();
      router.replace("/admin/login");
  ...
  const filteredContacts = contacts.filter(
    (c) =>
      ...
      (c.country &&
        c.country.toLowerCase().includes(contactSearch.toLowerCase())),
  );
    ...
    <Container className="py-10">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-accent to-[#002f6c] p-8 text-white shadow-lg mb-8">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            {greeting && (
              <span className="text-xs font-semibold tracking-wider text-accent-soft uppercase text-white">
                {greeting}
              </span>
            )}
            <h1 className="mt-1 text-3xl font-extrabold tracking-tight text-white leading-none">
              FIFA Sports Admin Portal
            </h1>
            <span className="mt-2 text-sm text-slate-200 max-w-xl text-white">
              Publish news items and manage submitted experiences or fan
              enquiries securely.
            </span>
          </div>
          <button
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          ...
          },
          {
            ...
            value: contacts.filter((c) => !c.approved).length,
          ...
          >
            ...
            <p className={`mt-1 text-3xl font-extrabold ${s.color}`}>
              {s.value}
            </p>
          </div>
        ))}
          ...
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-line pb-3 mb-4">
              <h2 className="text-lg font-bold text-ink">
                Active News Items ({filteredNews.length})
              </h2>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-bold text-ink">
                Shared Experiences & Enquiries
              </h2>
            </div>
            <p className="text-xs text-muted mt-1">
              Review contact form entries submitted by visitors.
            </p>
          </div>
                    ...
                    <>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div
                              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full font-bold text-sm text-white ${contact.approved ? "bg-green-500" : "bg-accent"}`}
                            >
                              {letter}
                            </div>
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5">
                                <p className="font-semibold text-ink leading-normal truncate max-w-xs">
                                  {contact.name}
                                </p>
                                {contact.approved && (
                              ...
                              <p className="text-xs text-muted leading-normal truncate max-w-xs">
                                {contact.email}
                              </p>
                            </div>
                          </div>
                        ...
                        >
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              onClick={() =>
                                onApproveContact(contact.id, !contact.approved)
                              }
                      ...
                      {isExpanded && (
                        <tr key={`${contact.id}-detail`}>
                          <td
                            colSpan={6}
                            className="bg-paper px-8 py-6 border-l-4 border-accent"
                          >
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
                                <div>
                                  ...
                                  <a
                                    ...
                                    className="mt-1 flex items-center gap-1.5 text-accent font-medium hover:underline"
                                  >
                                ...
                                {contact.phone && (
                                  <div>
                                    ...
                                    <a
                                      ...
                                      className="mt-1 flex items-center gap-1.5 text-ink font-medium hover:underline"
                                    >
